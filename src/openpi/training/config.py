...
class LeRobotBreadDataConfig(DataConfigFactory):
    ...
    action_sequence_keys: Sequence[str] = ("action",)
    # False -> the ra_bc_weight column is not threaded to the train step at
    # all (loader yields None -> uniform BC loss). Used by the *_bc control
    # configs so RA-BC vs BC differ ONLY in the loss weights, same pack.
    use_rabc_weight: bool = True
    # True -> the pack's cam_high (human-worn egocam) feeds base_0_rgb with
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        if self.use_high_cam:
            repack_keys["high_image"] = "observation.images.cam_high"
        if self.use_rabc_weight:
            repack_keys["ra_bc_weight"] = "ra_bc_weight"
        repack_transform = _transforms.Group(
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        data=LeRobotBreadDataConfig(
            repo_id="brianz/bread_r2",
            base_config=DataConfig(prompt_from_task=True),
        ),
        batch_size=32,
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=1_000, peak_lr=2.5e-5, decay_steps=96_000, decay_lr=2.5e-6,
        ),
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        num_train_steps=96_000,
        freeze_filter=pi0_config.Pi0Config(
            pi05=True, action_horizon=40, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora",
        ).get_freeze_filter(),
    ...
    # Uniform-BC control on the SAME r2 pack (weights column not threaded):
    # attributes any R2-vs-R2BC gap to the RA-BC weighting alone.
    TrainConfig(
        name="pi05_bread_r2_bc_lora",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=40, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora",
        ),
    ...
    # RA-BC v2 on the r4-top-50 subset (openpi_r2_top50: 47,079 frames), with
    # the high/ego cam feeding base_0_rgb. 44k steps = 30 epochs @bs32; sweep
    # ckpts from ~22k (15 ep) via offline eval, do NOT default to the last.
    # NOTE (exp notes): the 50 episodes are OUR r4 ranking (kept[:50] of
    # r1_top200_report), NOT Zhengmao's 50 -- success rates are not strictly
    # comparable arms. Training cam_high is the human-worn egocam; deploy
    # feeds the robot's fixed top RealSense (known view gap, deliberate bet).
    TrainConfig(
        name="pi05_bread_r2_rabc50_hicam",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=40, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora",
        ),
    ...
    # ---- R5 (2026-08-18): 50 RIGHT-hand-first bimanual eps (user-reviewed;
    # rand50 was 20L/30R mixed-handedness -> wrong-arm reach-across on robot),
    # 15 epochs, uniform BC, 20D relative_to_first state
    # (pack openpi_rand50, no inter-gripper) + chunk-relative rotvec actions.
    # cam_high KEPT (user call). Vs Zhengmao's recipe the only deltas are:
    # the random-50 draw, cam_high, and rot6d-vs-rotvec in the state encoding.
    # Serve with serve_bread_xctrl (chunk_relative auto; state stays rel_first).
    TrainConfig(
        name="pi05_bread_zm_right50",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=40, discrete_state_input=False,
            paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora",
        ),
    ...
    TrainConfig(
        name="pi05_bread_zm_right100_full",
        model=pi0_config.Pi0Config(
            pi05=True, action_horizon=40, discrete_state_input=False,
        ),
        data=LeRobotBreadDataConfig(
            repo_id="brianz/bread_right100",
            base_config=DataConfig(prompt_from_task=True),
            use_rabc_weight=False,
            use_high_cam=True,
            chunk_relative=True,
        ...
        batch_size=32,
        lr_schedule=_optimizer.CosineDecaySchedule(
            warmup_steps=1_000, peak_lr=2.5e-5, decay_steps=50_350, decay_lr=2.5e-6,
        ),
        optimizer=_optimizer.AdamW(clip_gradient_norm=1.0),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        num_train_steps=50_350,
        ema_decay=0.99,
        keep_period=5_000,
        num_workers=16,
    ),
